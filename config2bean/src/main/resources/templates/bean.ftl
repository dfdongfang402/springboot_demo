...
<#if !defineOnly>
    ...
    */
    public ${classname}(Row row,java.util.ArrayList
<String> collnames,String prefix,int index){
    <#if baseclass! !="" >
                ...
                <#list pr as collname>
                    ...
                    <#if myvar.name != "id">
                        <#if myvar.type! == "String" >
                            this.${myvar.name}=v;}
                        <#else>
                            if(v!=null) this.${myvar.name}=v; else throw new
                            IndexOutOfBoundsException(row.getRowNum()+"行的${collname}为空");}
                        </#if>
                    <#else>
