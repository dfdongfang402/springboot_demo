...
public class Xiaobangzi
{
	...
	private static class BeanMaker
	{
		...
		final private java.io.PrintStream makeOutPrintStream( final String filename)
		{
			try 
			{
				...
				return new java.io.PrintStream( new java.io.FileOutputStream(filename), true, "gbk");
			} 
	...
	private void writeManageCppFile( final String dstdir)
	{
		...
		try 
		{
			...
			new java.io.File(filename).getParentFile().mkdirs();
			ps = new java.io.PrintStream( new java.io.FileOutputStream(filename), true, "gbk");
		} 
		catch( final Exception ex) 
		{
			logger.error("err", ex);
	...
	static public void main( String[] args) throws Exception
	{
		...
		Object xpathresult = xpathFactory.newXPath().evaluate("//bean", doc, javax.xml.xpath.XPathConstants.NODESET);
		...
		for (int i = 0; i != nl.getLength(); ++i) 
		{
			...
			xiaobangzi.writeCppFile( dstdir, targetClassname, e);
		}
		xiaobangzi.writeManageCppFile( dstdir);
